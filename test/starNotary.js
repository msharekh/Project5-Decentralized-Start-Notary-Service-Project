...
it('can Create a Star', async () => {
  let tokenId = 1;
  // c('instance', instance);
  await instance.createStar('Awesome Star!', 'MSH', tokenId, {
    from: accounts[0]
  });
  let info = await instance.tokenIdToStarInfo.call(tokenId);
  let name = info[0];
  let symbol = info[1];
  // console.log('info', ':	', name);
  // console.log('symbol', ':	', symbol);
  assert.equal(name, 'Awesome Star!');
  assert.equal(symbol, 'MSH');
});

it('can get name of star', async () => {
  let tokenId = 1;
  // c('instance', instance);
...
it('lets user1 put up their star for sale', async () => {
  ...
  await instance.createStar('awesome star', 'MSH', starId, { from: user1 });
